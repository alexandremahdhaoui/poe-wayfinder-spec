...
The single source of truth for poe-wayfinder config and network policy.
...
| Path | Holds |
|---|---|
| `spec/poe-wayfinder.v1.yaml` | every config key of every binary |
...
`poe-wayfinder-app` resolves this file at build time, then runs
`golden-configgen` over it to emit `zz_generated.config.rs`.

```sh
sh ../poe-wayfinder-spec/hack/resolve-spec.sh \
...
its own key list, so `poe-wayfinder-datagen` never grows a hotkey setting.
